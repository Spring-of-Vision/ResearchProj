...
def unbin_packets(df):
    ...
    exploded = (
        df.apply(
            lambda ser: ser.explode().astype(np.int64) if ser.name in packet_cols else ser #.astype(np.int64) /int
        )
    )
...
def dominating_flow(df, threshold=0.9):
    ...
    pair_props = df.index.value_counts(normalize=True) #.droplevel(2)
...
def preprocess(
    ...
    indexed = index_by_flow(df) #unbinned
