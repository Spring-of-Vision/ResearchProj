    ...
    while True:
        if i*batch_size >= len(file_list):
            ...
            for file in file_chunk:
                ...
                arr = np.load(file) #[:,:,dimensions_to_use] #eliminated dimensions to use
                data.append(arr.reshape(src.INPUT_SHAPE))

                # One-hot encoding for labels
                label = [0, 0, 0]
                if os.path.dirname(file).endswith(b'GoogleDrive'):
                    label[0] = 1
                elif os.path.dirname(file).endswith(b'GoogleDoc'):
                    label[1] = 1
                elif os.path.dirname(file).endswith(b'Youtube'):
                    label[2] = 1
                labels.append(label)
...
def get_dataset(file_list, batch_size, dimensions_to_use, for_prediction=False):
    ...
    if for_prediction:
        ...
        dataset = tf.data.Dataset.from_generator(
            ...
            output_shapes=((None, *src.INPUT_SHAPE), (None, 3)) #updated label shape
        )

    steps_per_epoch = np.int64(np.ceil(len(file_list) / batch_size))
